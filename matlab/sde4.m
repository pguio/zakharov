...
%
% $Id: sde4.m,v 1.10 2011/03/26 09:20:42 patrick Exp $
%
...
if 0
	...
	plot(s.t,s.phit2)
	ylabel('|\phi(t)|^2');
	title(sprintf('sde4: %d paths @ %d points (c=%.1g), rng=%s, k=%.2g', ...
...
xlabel('t');
ylabel('|\phi(t)|^2')
title(sprintf('sde4: %d loops @ %d points, k=%.2g', s.nloop, s.maxiter,s.k))
for i=1:s.nmethod,
...
s.b = [s.phi0; s.source(1:end-1); s.dphi0; s.source2(1:end-1)];
