...
%
% $Id: sde5.m,v 1.24 2023/02/23 08:57:34 patrick Exp $
%
...
if nargin == 0 | (nargin == 1 & ~isfield(s,'dphi0')),
	s.dphi0 = sqrt(2*s.nu.*s.k.^2./(s.k.^2+4*s.nu.^2)).*s.k./sqrt(2*s.nu).*abs(s.phi0);
end
if nargin == 0 | (nargin == 1 & ~isfield(s,'D')),
  s.D = { sqrt(2*s.nu.*s.k.^2./(s.k.^2+4*s.nu.^2)).*abs(s.phi0), [0 0 0 0] };
end
...
% explicit method
	...
	s.dphi(1,:) = s.dphi0;
	for t = 2:s.maxiter,
		s.phi(t,:) = s.phi(t-1,:).*s.nu1 + s.dphi(t-1,:).*s.nu2 + s.source{1}(t,:);
		s.dphi(t,:) = s.dphi(t-1,:).*s.nu4-...
			s.h/2*s.k.^2.*(s.phi(t-1,:).*s.nu4+s.phi(t,:)) + s.source{2}(t,:);
	end
%phi1=s.phi; dphi1=s.dphi;
end
...
%phi2=s.phi; dphi2=s.dphi;
%fprintf(1,'%g ', mean(abs(phi1).^2-abs(phi2).^2)); fprintf(1,'\n');
%fprintf(1,'%g ', mean(abs(dphi1).^2-abs(dphi2).^2)); fprintf(1,'\n');
end
if 0
subplot(221), plot(s.t,abs(phi1(:,2)).^2,'g',s.t,abs(phi2(:,2)).^2,'r'); 
subplot(223), plot(s.t,abs(phi1(:,2)).^2-abs(phi2(:,2)).^2,'r');
subplot(222), plot(s.t,abs(dphi1(:,2)).^2,'g',s.t,abs(dphi2(:,2)).^2,'r'); 
subplot(224), plot(s.t,abs(dphi1(:,2)).^2-abs(dphi2(:,2)).^2,'r');
pause
end
...
for i=1:s.nk,
	...
	title(sprintf('\\nu=%.2g k=%.2g |n0|=%.2g |v0|=%.2g D_n=%.2g D_v=%.2g ', ...
...
text(0.5,0.99, sprintf('sde5.m: %d paths @ h=%g T=%g', s.ns,s.h,s.h*s.maxiter), ...
