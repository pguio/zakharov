...
%
% $Id: sde3.m,v 1.15 2011/03/26 09:20:42 patrick Exp $
%
...
if 0
	...
	plot(s.t,s.phit2)
	ylabel('|\phi(t)|^2');
	title(sprintf('sde3: %d paths @ %d points (c=%.1g), rng=%s, k=%.2g', ...
...
for j = 1:s.maxiter-1, 
	s.phi(j+1) = s.phi(j).*s.nu1+s.dphi(j)*s.nu2;
	s.dphi(j+1) = s.dphi(j)*s.nu4-(s.h/2)*s.k.^2.*(s.phi(j).*s.nu4+...
							s.phi(j+1))+s.source2(j);
end
phi1 = s.phi.*conj(s.phi);
dphi1 = s.dphi.*conj(s.dphi);
for j = 1:s.maxiter-1,
	s.phi(j+1) = s.phi(j)+s.dphi(j)*s.h;
	s.dphi(j+1) = (1-2*s.gamma*s.h)*s.dphi(j)-s.k^2*s.phi(j)*s.h+s.source2(j);
end
...
xlabel('t');
ylabel('|\phi(t)|^2')
title(sprintf('sde3: %d loops @ %d points, k=%.2g', s.nloop, s.maxiter,s.k))
for i=1:s.nmethod,
...
s.b = [s.phi0; zeros(1:s.maxiter-1,1); s.dphi0; s.source2(1:end-1)];
