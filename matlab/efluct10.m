function solver=efluct10
% function solver=efluct10

%
% $Id: efluct10.m,v 1.6 2011/03/26 09:20:41 patrick Exp $
%
...
solver.type='efluct';
solver.E0=10.0; % initial E-field power
solver.Erms=1e-2; % initial E-field fluctuation
...
if 0
	...
	solver.nui=sqrt(pi/8)*(sqrt(1/solver.mass_ratio)+solver.t_ratio^1.5* ...
  	exp(-0.5*solver.t_ratio-1.5))*k+solver.nuic;
	...
	solver.nue=zeros(size(k))+soler.ecoll/2;
	ii=find(k~=0);
	solver.nue(ii)=(s1./(k2(ii).*k(ii))).*exp(-s2./k2(ii))+solver.ecoll/2;
else
	solver.nui=0;
	solver.nue=0;
end
