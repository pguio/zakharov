function s=zak5(paramfile)
% function s=zak5(paramfile)

%
% $Id: zak5.m,v 1.2 2011/03/26 09:20:42 patrick Exp $
%
...
if 0
  ...
  s.w = 0.0;
	...
	ftn1 = ftn1sub(ftn0,ftdn0,fte0,s);

	fte1 = fte1sub(ftn1,ftn0,fte0,s);

	ftdn0 = ftdn0sub(ftdn0,fte0,ftn1,ftn0,fte1,s);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function ftn1=ftn1sub(ftn0,ftdn0,fte0,s)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

fte02 = fft(abs(s.edrive*exp(-i*(s.domega+s.w)*s.t0)+ifft(fte0)).^2);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function fte1=fte1sub(ftn1,ftn0,fte0,s)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
n0 = ifft(ftn0);
e0 = ifft(fte0);
ftne0 = fft(n0.*(s.edrive*exp(-i*(s.domega+s.w)*s.t0)+e0));

ftf = zeros(size(ftn1));
ftg = zeros(size(ftn1));

m = [1:s.mmax]'; % 2*mmax+1 modes calculated
e = exp(-(imh*(s.pi2ndx2*m.^2) + s.nye(m)*s.h));
ftf(m+1)     = e.*fte0(m+1);
...
e1 = (f-c*(g+s.edrive*exp(-i*(s.domega+s.w)*(s.t0+s.h))*n1-n1e0m))./(1+c*n1);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function ftdn0 = ftdn0sub(ftdn0,fte0,ftn1,ftn0,fte1,s)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

e0 = ifft(fte0);
fte02 = fft(abs(s.edrive*exp(-i*(s.domega+s.w)*s.t0)+e0).^2);


e1 = ifft(fte1);

fte12 = fft(abs(s.edrive*exp(-i*(s.domega+s.w)*(s.t0+s.h))+e1).^2);
