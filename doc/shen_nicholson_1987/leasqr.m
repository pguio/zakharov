function [f,p,kvg,iter,corp,covp,covr,stdresid,Z,r2,ss]=leasqr(x,y,pin,func,stol,niter,wt,dp,dfdp,options)
...
%prt=feval(dfdp,x,fbest,pbest,dp,func);
%N1_1=inv(2*prt'*prt)
%Q1=(2*prt'*(y-f))
%fprintf(1,'%.4e %.4e\n',((2*prt'*prt)\(-2*prt'*(y-f)))'*Q1)
%pause
...
for iter=1:niter,
  ...
  if verbose
    fprintf(1,'iter %d ,\tpi ',iter);
    fprintf(1,'%.4f\t', pbest);
    fprintf(1,', Phi(pi) %6f ', sbest);
    fprintf(1,'\n');
  end
...
%pause
  ...
  disp(r2)
  fprintf(1,'95%% conf region: F(0.05)(%.0f,%.0f)>= delta_pvec''*Z*delta_pvec\n',...
	n,m-n);
  disp(Z)
end;
